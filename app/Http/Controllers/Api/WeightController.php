...
class WeightController extends Controller
{
    ...
    // public function getPreview($id) // ← parameter langsung jadi $id
    // {
    //     // Pastikan user hanya bisa lihat dari timbangan miliknya
    //     $espId = session('current_esp_id');

    //     if (!$espId) {
    //         return response()->json([
    //             'success' => false,
    //             'message' => 'Timbangan belum dipilih'
    //         ], 403);
    //     }

    //     $cacheKey = "weight_preview_{$espId}_{$id}";
    //     $berat    = Cache::get($cacheKey, 0.0);

    //     return response()->json([
    //         'success' => true,
    //         'berat'   => round((float) $berat, 3) // kirim 3 desimal biar smooth
    //     ]);
    // }

    // ESP32 cek perintah (current_id + tare sekali pakai)
