        ...
        (function () {
            ...
            function renderNike(targetEl, targetBadge, rows, idPrefix) {
                ...
                function renderNikeGroup(containerEl, rowSet, groupLabel, groupColor, gPrefix) {
                    ...
                    const datePages = sortedDates.map(tgl => {
                        ...
                        return { date: tgl, pages: chunkArray(allRows, ROWS_PER_PAGE) };
                    });
            ...
            function renderNonNike(targetEl, targetBadge, orders, idPrefix) {
                    ...
                    for (let row = 0; row < ROWS_PER_BLOCK; row++) {
                        ...
                        padded.forEach(t => {
                            if (t) {
                                ...
                                let cls = 'w-ok';
                                if (mn > 0 && mx > 0) { if (bv < mn) cls = 'w-kurang'; else if (bv > mx) cls = 'w-lebih'; }
                ...
                function renderGroup(containerEl, blocks, groupLabel, groupColor, gPrefix) {
                    ...
                    const datePages = sortedDates.map(tgl => ({ date: tgl, pages: chunkArray(byDate[tgl], BLOCKS_PER_PAGE) }));
            ...
            function printNonNikePage(pageNum, pageBlocks) {
                ...
                pageBlocks.forEach(block => {
                    ...
                    for (let row = 0; row < ROWS; row++) {
                        ...
                        padded.forEach((t, ci) => {
                            ...
                            if (t) {
                                ...
                                const mx = parseFloat(t.rasio_batas_beban_max || 0);
                                let sty = '';
                                if (mn > 0 && mx > 0) { if (bv < mn) sty = 'color:red;font-weight:bold;'; else if (bv > mx) sty = 'color:orange;font-weight:bold;'; }
                                tdWeights += `<td style="${sty}">${bv.toFixed(2)}</td>`;
                            } else { tdWeights += `<td style="color:#ddd;">-</td>`; }
                        });

                        tbodyRows += `<tr><td rowspan="2" style="font-size:7px;color:#555;vertical-align:middle;white-space:nowrap;">${rowDate}</td>${tdBoxes}<td rowspan="2" style="font-weight:700;vertical-align:middle;">${hasData ? rowTotal.toFixed(2) : '-'}</td>` +
                    ...
                    const totalBerat = block.timbangans.reduce((s, t) => s + parseFloat(t?.berat || 0), 0).toFixed(2);
                    const checkingBadge = block.checking_ke > 1 ? `<span style="background:#ff6b35;color:#fff;padding:1px 6px;border-radius:3px;font-size:7px;">Checking #${block.checking_ke}</span>` : '';

                    blocksHTML += `<div class="block-wrap"><div class="block-title">CARTON WEIGHT REPORT &nbsp;—&nbsp; Laporan Timbangan Karton${isContinued ? ' <span style="background:#fff3cd;color:#856404;padding:1px 6px;border-radius:3px;font-size:8px;">Lanjutan</span>' : ''}${checkingBadge}</div>` +
            ...
            document.addEventListener('click', async function (e) {
                ...
                try {
                    ...
                    if (json.success) {
                        ...
                        if (statusEl) { statusEl.textContent = '✓ Tersimpan'; statusEl.style.color = 'green'; statusEl.style.display = 'inline'; setTimeout(() => { statusEl.style.display = 'none'; }, 2500); }
